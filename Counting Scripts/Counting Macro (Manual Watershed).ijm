...
*/		
		...
		//Interactive Watershed (threshold + watershed) (manual) (to calibrate on only the first image, remove the "//" before the "if" and the "}")
		//if (i < 1) {
		Dialog.createNonBlocking("Go to Interactive Watershed")
		Dialog.addMessage("Go to Interactive Watershed \n Click OK when you have set your parameters \n Increasing Seed Dynamics decreases the amount of segmentation \n Increasing Threshold does the same thing as Fiji's typical threshold \n Peak Flooding affects how full the segmentation looks")
		Dialog.show()
		Dialog.create("Input the settings from the Watershed");
			Dialog.addNumber("Seed Dynamics", "");
			Dialog.addNumber("Intensity Threshold", "");
			Dialog.addNumber("Peak Flooding", 100);
			Dialog.show();
			hMin = Dialog.getNumber();
			thresh = Dialog.getNumber();
			peakFlooding = Dialog.getNumber();
		//	}
		selectImage(image_file + " (blue)");
